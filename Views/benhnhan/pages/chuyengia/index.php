...
<head>
    ...
    <style>
        /* ===== Reset & layout fixes =====
           Fixes applied:
           - Remove body margin-top (it could hide header/H1 when a site header is fixed).
           - Use a main wrapper with top padding if you have a fixed site header.
           - Remove duplicated/conflicting .search-forms rules.
           - Ensure h1 is visible and the filter form doesn't overlap it.
         */
        html,body{
            height:100%;
            margin:0;
        ...
        /* If your site has a fixed header of ~100px height, set this on the page wrapper,
           so content is pushed below the header but not hidden by margins. */
        .page-wrapper {
            padding-top: 100px; /* adjust only if your site header is fixed; otherwise set to 0 */
        ...
        .filter-form .input-group .bi-search {
            position: absolute;
            left: 12px;
            color: #6b2f8a;
            font-size: 0.9rem;
        ...
        .filter-form .form-control:focus{
            border-color: rgba(108,58,148,0.3);
            box-shadow: 0 2px 12px rgba(59,21,97,0.12);
        }
        ...
        @media (max-width: 576px){
            ...
            .filter-form { gap: 8px; padding: 0 10px; }
        }
        ...
        .btn-secondary { background-color: #0288d1; color: #fff; }
...
<body>
<div class="page-wrapper"> <!-- use this wrapper to handle fixed site-header if any -->
    ...
    ?>
        <div class="doctor-card" role="article" aria-label="<?php echo htmlspecialchars($row['hoten']); ?>">
            ...
            <div class="doctor-info">
                ...
                <div class="doctor-buttons" >
                    ...
                    <a href="?action=lichkhamchuyengia&idcg=<?php echo $row['machuyengia']; ?>" class="btn-schedule" aria-label="Xem lịch khám bác sĩ <?php echo htmlspecialchars($row['hoten']); ?>">XEM LỊCH KHÁM</a>
                </div>
            </div>
        </div>
    <?php
