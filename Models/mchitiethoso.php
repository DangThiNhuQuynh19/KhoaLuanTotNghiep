...
if(isset($_POST['btnHoanTat']) || isset($_POST['btnupdate'])) {
    ...
    $posted_ngay = $_POST['ngaykham'] ?? null; // <- capture visit date from modal
    ...
    $createResult = null;
    try {
        $createResult = $cchitiethoso->create_chitiethoso($posted_mahoso, $bacsi['mabacsi'], $trieuchung, $chandoan, $huongdieutri, $madonthuoc, $ketluan);
    ...
    if($createResult ){
        ...
        if(!empty($posted_maphieu)){
            $resStatus = $cphieukhambenh->updateTrangThaiPKB($posted_maphieu, 'Đã khám');
            if(!empty($posted_ngay)){
                if (method_exists($cphieukhambenh, 'updateNgayKham')) {
                    $cphieukhambenh->updateNgayKham($posted_maphieu, $posted_ngay);
                } elseif (method_exists($cphieukhambenh, 'update_ngaykham')) {
                    $cphieukhambenh->update_ngaykham($posted_maphieu, $posted_ngay);
                } elseif (method_exists($cphieukhambenh, 'updateNgay')) {
                    $cphieukhambenh->updateNgay($posted_maphieu, $posted_ngay);
                }
            }
            $okUpdate = (bool)$resStatus;
        } else {
            $okUpdate = true;
        }
    } else {
...
document.querySelectorAll('.btn-kham').forEach(function(btn){
   btn.addEventListener('click', function(){
       ...
       var dataNgay = btn.getAttribute('data-ngay');

       // Prefer using the appointment date if provided; otherwise set to current date
       var dateToSet;
       if (dataNgay) {
           // attempt to normalize date format (supports yyyy-mm-dd or other)
           var d = new Date(dataNgay);
           if (!isNaN(d.getTime())) {
       ...
       // set ngày khám to appointment date if available, otherwise current date
       document.getElementById('form_ngay').value = dateToSet;
